...
def visualize_predictions(data):
    """Visualize predicted efficacy scores and biological feature distributions."""
    
    # 1. Distribution of Predicted Efficacy Scores
    plt.figure(figsize=(10, 6))
    sns.histplot(data['predicted_efficacy'], kde=True, color='skyblue')
    plt.title('Distribution of Predicted Efficacy Scores')
    ...
    plt.figure(figsize=(10, 6))
    sns.boxplot(data=data[['GC_content', 'Tm', 'entropy']])
    plt.title('Boxplot of GC Content, Tm, and Entropy')
    plt.grid(True)
    plt.show()
...
if __name__ == "__main__":
    ...
    model_file = "Stacking Ensemble_model.pkl"  # Replace with your saved model file path

    predicted_data = find_guides_with_pam(fasta_file, model_path=model_file)
