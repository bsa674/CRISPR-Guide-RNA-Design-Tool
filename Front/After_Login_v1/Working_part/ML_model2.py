...
def main_pipeline(filepath):
    ...
    for model_name, model in models.items():
        trained_model, mse, r2 = train_and_evaluate_model(model, X_train, y_train, X_test, y_test, model_name)
        if r2 > best_r2:
            best_r2 = r2
    ...
    if best_model:
        ...
        logger.info(f"Best model ({best_model_name}) saved as '{best_model_name}_model.pkl'.")

    # Save visualizations
    output_dir = datetime.now().strftime("visualizations_%Y%m%d_%H%M%S")
    ...
    plt.savefig(os.path.join(output_dir, "biological_features_distribution.png"))
    plt.close()

    logger.info(f"Visualizations saved in directory: {output_dir}.")

    end_time = datetime.now()
    logger.info(f"Pipeline completed at {end_time}. Total execution time: {end_time - start_time}.")

#if __name__ == "__main__":
if os.path.exists('Working_part/data_enc.csv'):
#    print(os.getcwd())

    main_pipeline('Working_part/data_enc.csv')
else:
    print(os.getcwd())
